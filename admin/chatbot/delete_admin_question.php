...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM admin_questions WHERE id = $delete_id";
    if (mysqli_query($connection, $delete_query)) {
        header('Location: delete_admin_question.php');
        exit;
    } else {
...
// Query to fetch questions and responses from the admin_questions table
$query = "SELECT id, question, response FROM admin_questions";
                            ...
                            <div class="card-header bg-white border-0">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-4 text-right">
                                        <a href="create_question.php" class="btn btn-md btn-primary">Create New</a>
                                    </div>
                                </div>
                            </div>
                            ...
                            <table class="table table-striped table-hover table-fw-widget" id="table3">
                                ...
                                <tbody>
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        echo "<td><span class='truncate'>{$row['response']}</span></td>";
                                        echo "<td class='text-center'>"; // Adjusted the class here
                                        echo "<div class='btn-group'>"; // Added a div for button group
                                        echo "<button type='button' class='btn btn-flat btn-primary dropdown-toggle dropdown-icon' data-toggle='dropdown'>
                                                Action
                                                <span class='sr-only'>Toggle Dropdown</span>
                                            </button>";
                                        echo "<div class='dropdown-menu' role='menu'>";
                                        echo "<a class='dropdown-item' href='edit_question.php?id={$row['id']}'><span class='fa fa-edit text-primary'></span> Edit</a>";
                                        echo "<div class='dropdown-divider'></div>";
                                        echo "<a class='dropdown-item delete_data' href='delete_admin_question.php?delete_id={$row['id']}' data-id='{$row['id']}'><span class='fa fa-trash text-danger'></span> Delete</a>";
                                        echo "</div>";
