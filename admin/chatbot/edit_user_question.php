...
if ($id && is_numeric($id)) {
    // Fetch the existing user-submitted question
    $query = "SELECT question FROM user_questions WHERE id = $id";
    ...
    echo "Invalid question ID."; // Handle the case where $id is not provided or is not a valid integer.
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $adminResponse = $_POST['admin_response'];

    // Check if the admin response is not blank
    if (!empty($adminResponse)) {
        // Insert the admin response into the admin_questions table
        $insertQuery = "INSERT INTO admin_questions (question, response) VALUES ('$userQuestion', '$adminResponse')";

        if (mysqli_query($connection, $insertQuery)) {
            // After inserting into admin_questions, you can also delete the question from user_questions
            $deleteQuery = "DELETE FROM user_questions WHERE id = $id";
            if (mysqli_query($connection, $deleteQuery)) {
                // Redirect to admin_questions.php or another page as needed
                header('Location: admin_questions.php');
                exit;
            } else {
                echo "Error deleting the question: " . mysqli_error($connection);
            }
        } else {
            echo "Error adding response: " . mysqli_error($connection);
        }
    } else {
        echo "Response cannot be blank.";
    }
}
?>
            ...
            <div class="page-head">
                ...
                <nav aria-label="breadcrumb" role="navigation">
                    <ol class="breadcrumb page-head-nav">
                        ...
                        <li class="breadcrumb-item active">Create New Response to User Question</li>
                    </ol>
                </nav>
                <br><br>
                <!----------------------------------------------------------------------------------------------------------------------->
                ...
                <div class="card card-outline card-info">
                    <div class="card-header">
                        <h3 class="card-title">Create New Response to User Question</h3>
                    </div>
                    <?php if (!empty($error)) : ?>
                        ...
                        <form method="post" action="" id="response-form">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group col-lg-6">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="new_question" name="new_question" value="<?php echo $userQuestion; ?>" disabled>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="admin_response" name="admin_response" class="form-control" rows="4" cols="50"></textarea>
